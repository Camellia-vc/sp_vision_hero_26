  ...
  auto plan_thread = std::thread([&]() {
    ...
    while (!quit) {
      if (!target_queue.empty() && mode == io::GimbalMode::AUTO_AIM) {
        ...
        data["plan_distance"] = plan.distance;
        ...
        if (target.has_value()) {
          ...
          if (!armor_xyza_list.empty()) data["armor_0_z"] = armor_xyza_list[0][2];
          if (armor_xyza_list.size() > 1) data["armor_1_z"] = armor_xyza_list[1][2];
          if (armor_xyza_list.size() > 2) data["armor_2_z"] = armor_xyza_list[2][2];
          if (armor_xyza_list.size() > 3) data["armor_3_z"] = armor_xyza_list[3][2];
        }
  ...
  while (!exiter.exit()) {
    ...
    /// 打符
      ...
      gimbal.send(
        ...
        buff_plan.pitch, buff_plan.pitch_vel, buff_plan.pitch_acc, buff_plan.distance);
