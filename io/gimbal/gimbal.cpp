...
void Gimbal::send(io::VisionToGimbal VisionToGimbal)
{
  ...
  tx_data_.distance = VisionToGimbal.distance;
  ...
  float pitch_acc, float distance)
{
  ...
  // std::cout << "serial:" << pitch << std::endl;
  tx_data_.pitch_vel = pitch_vel;
  tx_data_.pitch_acc = pitch_acc;
  tx_data_.distance = distance;
  tx_data_.crc16 = tools::get_crc16(
    reinterpret_cast<uint8_t *>(&tx_data_), sizeof(tx_data_) - sizeof(tx_data_.crc16));
...
void Gimbal::send(const io::Command & command)
{
  send(
    ...
    static_cast<float>(command.pitch), 0.0f, 0.0f, static_cast<float>(command.horizon_distance));
}
