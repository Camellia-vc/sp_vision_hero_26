  ...
  bool to_now)
{
  ...
  if (bullet_speed < 10.0  || bullet_speed > 12.5) bullet_speed = 11.5;
...
AimPoint Aimer::choose_aim_point(const Target & target)
{
  ...
  // ---- Outpost: plate selection for prediction ----
  // EKF models all 3 plates (green boxes), prediction feeds only the one
  // closest to the camera (red box).  Switching is gated by a 45° deviation
  // threshold and 5-frame hysteresis to prevent oscillation.
  if (target.name == ArmorName::outpost) {
    // Find plate closest to camera (origin).
    int closest_id = 0;
    double closest_d2 = std::numeric_limits<double>::max();
    for (int i = 0; i < static_cast<int>(armor_num); i++) {
      double d2 = armor_xyza_list[i][0] * armor_xyza_list[i][0] +
                  armor_xyza_list[i][1] * armor_xyza_list[i][1];
      if (d2 < closest_d2) { closest_d2 = d2; closest_id = i; }
    }

    // Initialise lock on first invocation.
    if (outpost_aim_lock_id_ < 0 || outpost_aim_lock_id_ >= static_cast<int>(armor_num)) {
      outpost_aim_lock_id_ = closest_id;
      outpost_aim_lock_frames_ = 999;
    }

    // Deviation of the currently-locked plate.
    double lock_angle = armor_xyza_list[outpost_aim_lock_id_][3];
    double lock_to_cam = std::atan2(
      -armor_xyza_list[outpost_aim_lock_id_][1],
      -armor_xyza_list[outpost_aim_lock_id_][0]);
    double lock_dev = std::abs(tools::limit_rad(lock_to_cam - lock_angle));

    constexpr double kSwitchAngle = 45.0 * CV_PI / 180.0;
    constexpr int kMinFrames = 5;

    bool should_switch = false;
    if (closest_id != outpost_aim_lock_id_) {
      double close_angle = armor_xyza_list[closest_id][3];
      double close_to_cam = std::atan2(
        -armor_xyza_list[closest_id][1],
        -armor_xyza_list[closest_id][0]);
      double close_dev = std::abs(tools::limit_rad(close_to_cam - close_angle));

      // Switch when the locked plate has turned too far away, OR when the
      // closest plate is more camera-facing than the locked one.
      should_switch = (lock_dev > kSwitchAngle || close_dev < lock_dev) &&
                      outpost_aim_lock_frames_ >= kMinFrames;
    }

    if (should_switch) {
      outpost_aim_lock_id_ = closest_id;
      outpost_aim_lock_frames_ = 0;
    } else {
      outpost_aim_lock_frames_ = std::min(outpost_aim_lock_frames_ + 1, kMinFrames + 1);
    }

    return {true, armor_xyza_list[outpost_aim_lock_id_]};
  }
