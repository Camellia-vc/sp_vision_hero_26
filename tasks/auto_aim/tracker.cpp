...
bool Tracker::set_target(std::list<Armor> & armors, std::chrono::steady_clock::time_point t)
{
  ...
  if (is_balance) {
    solver_.solve(armor);
    Eigen::VectorXd P0_dig{{1, 64, 1, 64, 1, 64, 0.4, 100, 1, 1, 1, 0, 0}};
    ...
    for (auto & matched_armor : matched_armors) {
      solver_.solve(matched_armor);
    }

    Eigen::VectorXd P0_dig{{1, 64, 1, 64, 1, 81, 0.4, 100, 1e-4, 0, 1e-2, 1e-2, 1e-2}};
    target_ = Target(matched_armors.front(), t, 0.2765, 3, P0_dig);
    target_.update(matched_armors);
  }

  else if (armor.name == ArmorName::base) {
    solver_.solve(armor);
    Eigen::VectorXd P0_dig{{1, 64, 1, 64, 1, 64, 0.4, 100, 1e-4, 0, 0, 0, 0}};
...
bool Tracker::update_target(std::list<Armor> & armors, std::chrono::steady_clock::time_point t)
{
  ...
  if (target_.name == ArmorName::outpost) {
    ...
    for (const auto & armor : matched_armors) target_.update(armor);
  }
